...
def visualizeBatchPair(options, config, inp_pair, detection_pair, indexOffset=0, prefix='', suffix='', write_ply=True, write_new_view=False):
    detection_images = []    
...
def visualizeBatchDetection(options, config, input_dict, detection_dict, indexOffset=0, prefix='', suffix='', prediction_suffix='', write_ply=True, write_new_view=False):
    image_dict = {}
...
#        error = torch.clamp(torch.abs(input_dict['depth'] - detection_dict['depth_np']) *(input_dict['depth']> 1e-4).float(),min=0.0,max=3)
#        error = torch.clamp(error, min=0.0,max=3)
#        plt.figure()
#        plt.imshow(error[:,80:560].squeeze().detach().cpu().numpy(), cmap = cm.tab10)
#        plt.colorbar()
#        plt.savefig(options.test_dir + '/' + str(indexOffset) + '_error' + suffix + prediction_suffix + '_np.png')
        pass
    ...
    if write_ply:
        ...
        print(options.test_dir + '/' + str(indexOffset) + '_model' + suffix + prediction_suffix +'_no_rgb' +'.ply')
        writePLYFileMask_norgb(options.test_dir + '/' + str(indexOffset) + '_model' + suffix + prediction_suffix + '_no_rgb'+'.ply', image[80:560], detection_masks[:, 80:560].detach().cpu().numpy(), detection_dict['plane_XYZ'].detach().cpu().numpy().transpose((0, 2, 3, 1))[:, 80:560], write_occlusion='occlusion' in options.suffix)
...
def visualizeBatchDepth(options, config, input_dict, detection_dict, indexOffset=0, prefix='', suffix='', write_ply=True):
    image_dict = {}
    images = input_dict['image'].detach().cpu().numpy().transpose((0, 2, 3, 1))
    images = unmold_image(images, config)
...
def writePLYFileMask_norgb(filename, image, masks, plane_XYZ, write_occlusion=False):
