...
#                check1,check2, check3 = calcXYZModule(self.config, camera, detection_dict['detection'], detection_masks,detection_dict['depth_np'], return_individual=True)
#                print(check1[1:2].shape)
#                print(check1.shape)
#                print(check1)
#                check1 = torch.nn.functional.interpolate(check1[:, 80:560].unsqueeze(1), size=(480, 640), mode='bilinear').squeeze(1)
#                print(check1.size())
#                print(check1)
#               
#                print((check1 [:, 80:560]).size())
#           
#                print(check1 [:, 80:560])
...
#def superpixels_histograms_neighbors(img,n_segments,compactness):
#    # SLIC
...
#    
##    img = np.full((640,640,3), 12, np.uint8)
#    
#    print(img.shape)
...
#
#
#def get_depth_error(data1,depth_image):
#    centers = data1[0]
#    color_hists = data1[1]
#    segments = data1[2]
#    ptr,ind =  data1[3]
...
#        print(std_i)
##        print(std_j)
#        N = ind[ptr[i]:ptr[i+1]] # list of neighbor superpixels
#        
...
#        for n in N :        
#
#            d_n = torch.abs(d_sp - torch.mean(depth_image[seg==int(n)]))            
...
def pc_from_xyz2(XYZ_pc, out_dir,img,model_type, label):    
    

    #reduce the digits of coordinates 
    XYZ = XYZ_pc[:, 80:560]
    shape = XYZ.shape
#    print(shape)
    
    #make a fake points 
    points = np.zeros(shape = (shape[1]*shape[2],shape[0]))
  
#    colors = np.zeros(shape = (shape[1]*shape[2],shape[0]))
    width = shape[1]
    height = shape [2]
#    depth1 = XYZ_pc[1:2]
#    depth1 = np.clip(depth1 / 5 * 255, 0, 255)
#    colors1 = cv2.applyColorMap(255 - depth1, colormap=cv2.COLORMAP_JET).reshape((-1, 3))
#    print(img.shape)
#    print(img)
    colors = []
    obj_label = []
#    colors = cv2.applyColorMap(img, colormap=cv2.COLORMAP_JET)
    
#    width = 3
#    height = 3
    for i in range(0,width):
        for j in range(0,height):
            points[height*i+ j][0] = float(XYZ[0][i][j])
            points[height*i+ j][1] = float(XYZ[2][i][j])
            points[height*i+ j][2] = -float(XYZ[1][i][j])    
            colors.append([img[i,j]])
#            obj_label.append(label[i,j])
    print(len(colors))
    print(len(points))
        
    print("inside")
    print("done")
    filename = out_dir +'/'+ str(model_type) +'.ply'
...
"""
            ...
            for point in points:
                ...
                continue
            f.close()
            print("The "+ str(model_type)+ "model is saved. ")
...
"""            
            ...
            for c in range(0,len(colors)):
                f.write(str(points[c][0]) + ' ' + str(points[c][1]) + ' ' + str(points[c][2]) + ' ' + str(colors[c][0][2]) + ' ' + str(colors[c][0][1]) + ' ' + str(colors[c][0][0])+'\n')            
                continue
            f.close()
            print("The "+ str(model_type)+ "model is saved. ")
...
"""
            f.write(header) 
            for c in range(0,len(colors)):
                f.write(str(points[c][0]) + ' ' + str(points[c][1]) + ' ' + str(points[c][2]) + ' ' + str(colors[c][0][2]) + ' ' + str(colors[c][0][1]) + ' ' + str(colors[c][0][0])+ ' ' + str(obj_label[c]) +'\n')            
                continue
            f.close()
            print("The "+ str(model_type)+ "model is saved. ")
    return 
...
def pc_from_xyz(XYZ_pc, out_dir,img,model_type, label):    
...
#                    visualizeBatchPair(options, config, input_pair, detection_pair, indexOffset=sampleIndex % 500, suffix='_' + name + options.modelType, write_ply=True, write_new_view=options.testingIndex >= 0 and 'occlusion' in options.suffix)
                    #create point cloud from predictions
             
                    label = [1]
                    ori_image = cv2.imread("example_images/126.png")            
                    ori_image = cv2.resize(ori_image,(640, 480))
                    
                   
#                  TO get a piecewise planar model \\to do create a ply file from obtained parameters  
                    
#                    info1 = np.array([camera[0], 0, camera[2], 0, 0, camera[1], camera[3], 0, 0, 0, 1, 0, 0, 0, 0, 1, camera[4], camera[5], 1000, 0])
#                    normals = calcNormal_spx(detection_pair[c]['depth'].squeeze().detach().cpu().numpy(), input_pair[c]['camera'].squeeze().detach().cpu().numpy())
#                    planes1, seg1 = fitPlanesPiecewise(ori_image, detection_pair[c]['depth'].squeeze().detach().cpu().numpy(), normals, info1, numOutputPlanes=20, imageIndex=1, parameters={})
                    
#                   
##                    XYZ_pc, detection_mask_t = calcXYZModule_rec(ori_image,1200,20,config, camera, detection_pair[c]['detection'], detection_pair[c]['masks'], detection_pair[c]['depth_np'], return_individual= False)
#                                         
                    model_type = [ "no_rgb_", "color_rgb_", "semantics_rgb_", "gt_spx_"]
                    
                    pc_type = ["g_np", "g_rec", "d_np", "d_rec" ]
                    
                    
#                    XYZ_g_np, detection_mask_t = calcXYZModule(config, camera, detection_pair[c]['detection'], detection_pair[c]['masks'], detection_pair[c]['depth_np'], return_individual= False)
#                    XYZ_d_np = get_gt_pc (config, camera,detection_pair[c]['depth_np'] )
#                   
#
##                    
###                    point cloud from model 
##                    
#                    label = [0]
#                    pc_from_xyz(XYZ_g_np,options.test_dir, ori_image, model_type[0]+pc_type[0], label)                    
#                    label = [1]
#                    pc_from_xyz(XYZ_g_np,options.test_dir, ori_image, model_type[1]+ pc_type[0], label)
#                    
#                                      
##                    depth from model
#                    label = [0]
#                    pc_from_xyz(XYZ_d_np,options.test_dir, ori_image, model_type[0]+ pc_type[2], label)  
#                    label = [1]
#                    pc_from_xyz(XYZ_d_np,options.test_dir, ori_image, model_type[1] + pc_type[2], label) 
##                    
###                    
                    depth_rec = rec_depth_cons(ori_image,1600,20,detection_pair[c]['depth_np']) 
                    XYZ_g_rec, detection_mask_t = calcXYZModule(config, camera, detection_pair[c]['detection'], detection_pair[c]['masks'], depth_rec, return_individual= False)
                    XYZ_d_rec = get_gt_pc (config, camera,depth_rec)
#

#                   point cloud after  consistency term  
                    
                    label = [0]
                    pc_from_xyz(XYZ_g_rec,options.test_dir, ori_image, model_type[0]+ pc_type[1], label)
                    label = [1]
                    pc_from_xyz(XYZ_g_rec,options.test_dir, ori_image, model_type[1]+ pc_type[1], label)  
  
                    
#                    depth after consistency term 
                    
                    label = [0]
                    pc_from_xyz(XYZ_d_rec,options.test_dir, ori_image, model_type[0]+ pc_type[3], label) 
                    label = [1]
                    pc_from_xyz(XYZ_d_rec,options.test_dir, ori_image, model_type[1] + pc_type[3], label) 
                    
                    
#                   ground truth  
                    d_gt = cv2.imread('gtp/126.png', -1).astype(np.float32) / 1000.0
                    d_gt = np.concatenate([np.zeros((80, 640)), d_gt, np.zeros((80, 640))], axis=0)    
                    d_gt = d_gt.astype(np.float32)
                    d_gt = torch.from_numpy(d_gt).unsqueeze(0).cuda()
#                   
                    depth_rec1 = rec_depth_cons(ori_image,1600,20,d_gt)
                    XYZ_gt = get_gt_pc (config, camera,depth_rec1)  
                   
#                    XYZ_gt = get_gt_pc (config, camera,d_gt)  
                    
                    label = [0]
                    pc_from_xyz(XYZ_gt,options.test_dir, ori_image, model_type[3]+model_type[0], label)
                    label = [1]
                    pc_from_xyz(XYZ_gt,options.test_dir, ori_image, model_type[3]+model_type[1], label)
#                
#                
#                
...
#                    pc_from_xyz(XYZ_pc,options.test_dir, ori_image, model_type[1], label) 
                    
#                    pc_from_xyz(XYZ_pc,options.test_dir, img_parse, model_type[2], label)
                    
#                    ranges1 = config.getRanges(camera)
#                    
#                    plane_parameters = detection_pair[c]['detection'][:, 6:9]
#                    
#                    
#                    normals_pre = planeXYZModule_rec(ori_image,1200,20,ranges1, plane_parameters, width=config.IMAGE_MAX_DIM, height=config.IMAGE_MIN_DIM)
                   #                    
#                    print(normals_pre)
#                    print(normals_pre.shape)
#                    print(normals_pre[0][0])
                    
                    # create gt point cloud

#                    d_gt = cv2.imread('gt/16.png', -1).astype(np.float32) / 1000.0
#                    d_gt = np.concatenate([np.zeros((80, 640)), d_gt, np.zeros((80, 640))], axis=0)    
#                    d_gt = d_gt.astype(np.float32)
#                    d_gt = torch.from_numpy(d_gt).unsqueeze(0).cuda()
#                    cv2.imwrite('p4/fig/' + 'd_gt_16.png', drawDepthImage(d_gt.squeeze().detach().cpu().numpy()[80:560]))
                   
#                    segment = cv2.imread('p4/seg.png', -1).astype(np.float32)
#                    cv2.imwrite('p4/fig/' + 'seg_16.png', drawSegmentationImage(segment))
...
#                   
                     ...
                     #error visualization
#                    error = torch.abs(d_gt - detection_pair[c]['depth_np']) *(d_gt> 1e-4).float()
#                    error = torch.clamp(error, min=0,max=1.8)
#                    drawerrorImage(error[:,80:560].squeeze().detach().cpu().numpy())
                    
                    # segementation visualization
#                    
#                    ori_image = io.imread('p4/16.jpg')           
#                    ori_image = cv2.resize(ori_image,(640, 480))
#                    sd_gt = torch.zeros(d_gt.shape).cuda()                    
#                    sd_pd = torch.zeros(detection_pair[c]['depth'].shape).cuda()    
#                    sd_pd,sd_gt = create_spx_depthmask(ori_image,800,20,detection_pair[c]['depth'], d_gt, sd_pd, sd_gt) 
#                    cv2.imwrite('p4/fig/' + 'd_gt_16.png', drawDepthImage(d_gt.squeeze().detach().cpu().numpy()[80:560]))
                    
#                    start = time.time()
#                    mean_pixel = np.array([123.7, 116.8, 103.9])          
                    
                    #saving error and mask od dpeth image 
                    
#                    cv2.imwrite('test/concept/' + 'error.png', fig)
                    
#                    cv2.imwrite('test/concept/' + 'gt_mean.png', drawDepthImage(d_gt.squeeze().detach().cpu().numpy()[80:560]))
                  
#                    cv2.imwrite('test/concept/' + 'gt_BLACK.png', drawDepthImage(d_gt.squeeze().detach().cpu().numpy()[80:560]))
##                    
#                    cv2.imwrite('test/concept/' + 'pre_BLACK.png', drawDepthImage(detection_pair[c]['depth_np'].squeeze().detach().cpu().numpy()[80:560])) 
#                    
                    
                    
#                    image33 = (input_pair[c]['image'][0].detach().cpu().numpy().transpose((1, 2, 0)) )[80:560]  
#                    image33 = unmold_image(image33, config)
#                    
#                   
##                    detection_pair[c]['depth_np'],d_gt = get_spx_depthmask(image33,1400,10,detection_pair[c]['depth_np'], d_gt,'mean')  
#                    
##                    print(detection_pair[c]['depth_np'][:,80:560] )
##                    new12 = l1LossMask(detection_pair[c]['depth'][:, 80:560], d_gt[:, 80:560], (d_gt[:, 80:560] > 1e-4).float())
#                    start = time.time()
#                    new11 = get_depth_loss_or(image33,1200,10,d_gt,detection_pair[c]['depth_np'], 'mean',0.1)
#                    print(new11)
##                    
##                    print(detection_pair[c]['depth_np'][:,80:560] )
...
#                    cv2.imwrite('test/concept/' + 'gt_mean14.png', drawDepthImage(d_gt.squeeze().detach().cpu().numpy()[80:560]))
                    

#                    cv2.imwrite('test/concept/' + 'pre_mean.png', drawDepthImage(detection_pair[c]['depth_np'].squeeze().detach().cpu().numpy()[80:560]))    
                    
#                    
#                    detection_pair[c]['depth_np'],d_gt = get_spx_depthmask(image33,1000,10,detection_pair[c]['depth_np'], d_gt,'center')  
#                    new12 = l1LossMask(detection_pair[c]['depth'][:, 80:560], d_gt[:, 80:560], (d_gt[:, 80:560] > 1e-4).float())
#                    print(new12)
#                    
#                    print(start - time.time())
##                    
#                    cv2.imwrite('test/concept/' + 'gt_cent10.png', drawDepthImage(d_gt.squeeze().detach().cpu().numpy()[80:560]))
##                    
#                    cv2.imwrite('test/concept/' + 'pre_cent.png', drawDepthImage(detection_pair[c]['depth_np'].squeeze().detach().cpu().numpy()[80:560])) 
#                    
 # loss calculation expt 
...
#                    print(input_pair[c]['image'][0])
#                    mean_pixel = np.array([123.7, 116.8, 103.9])   
#                    image33 = (input_pair[c]['image'][0].detach().cpu().numpy().transpose((1, 2, 0)) )[80:560]
#                    image33 = input_pair[c]['image'][80:560].detach().cpu().numpy().transpose((0, 2, 3, 1))
...
#                    sd_gt12 = torch.zeros(d_gt.shape).cuda()                    
#                    sd_pd12 = torch.zeros(detection_pair[c]['depth'].shape).cuda()                     
                                             
#                    sd_pd12,sd_gt12 = create_spx_mask(ori_image,1200,20,detection_pair[c]['depth'], d_gt, sd_pd12, sd_gt12)   
#                    new = l1LossMask(sd_pd[:, 80:560], sd_gt[:, 80:560], (sd_gt[:, 80:560] > 1e-4).float())


                   
#                    loss_mean = get_cerror_depthmask(image33,1200,10,detection_pair[c]['depth'], d_gt,'mean') 
...
#                    
                    # plot_depth_dist
#                    create_dist(image33,1000,20,detection_pair[c]['depth'][:, 80:560][0])




#                    depth_ref_loss = get_depth_error(image33,1000,20,detection_pair[c]['depth'][:, 80:560][0])
                    
#                    print('depth_smooth_loss',depth_ref_loss)
                    
#                    end = time.time()
#                    print('losstime', end-end1)
                    
#                    print('totaltime', end-start)
                    
#                    print(depth_ref_loss[0])
#                    print(depth_ref_loss[1])
#                    print(depth_ref.shape)
                    
#                    depth_ref2 = detection_pair[c]['depth_np'][:, 80:560].float()
                   
                    
#                    depth_image = img_as_float(io.imread("example_images/depth.png"))
                    
#                    print(depth_image.shape)                 
                    
                    
#                    img_ori = cv2.imread(options.customDataFolder + "/0_image_0.png")
                    
#                    img_parse = cv2.imread(options.test_dir + "/02406.png")
                    
#                    label = np.load(options.test_dir + "/labels_02406.npy")   
                    
#                    label = [1]
#                    
#                    model_type = [ "no_rgb_", "color_rgb_", "semantics_rgb_", "gt"]
#                    
#                    ori_image = cv2.imread("example_images/4.jpg")            
#                    ori_image = cv2.resize(ori_image,(640, 480))
                    
#                    pc_from_xyz(xyz_gt,options.test_dir, image33, model_type[3], label)
#                    pc_from_xyz(XYZ_pc,options.test_dir, img_ori, model_type[0], label)
#                    pc_from_xyz(XYZ_pc,options.test_dir, ori_image, model_type[1], label)                    
#                    pc_from_xyz(XYZ_pc,options.test_dir, img_parse, model_type[2], label)
    ...
    if 'inference' not in options.dataset:
        ...
        if options.testingIndex == -1:
            if(args.dataFolder== 'scan_curve'):
                np.save('logs/all_statistics_curve.npy', all_statistics)
            elif(args.dataFolder== 'scan_plane'):
                np.save('logs/all_statistics_plane.npy', all_statistics)
            else:
...
if __name__ == '__main__':
    ...
    if args.dataset == '':
        if(args.dataFolder== 'scan_curve'):
            args.keyname = 'eval_curve'
        elif(args.dataFolder== 'scan_plane'):
            args.keyname = 'eval_plane'
    else:
