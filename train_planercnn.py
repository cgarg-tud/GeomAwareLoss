...
def train(options):
    ...
    print('options.numEpochs',options.numEpochs)
...
#                start= time.time()
                s_img = (images[0].detach().cpu().numpy().transpose((1, 2, 0)) + mean_pixel)[80:560]                
                prop = superpixels_data(s_img,800,20)
                depth_smooth_loss = get_depth_error(s_img,800,20,depth_np_pred[:, 80:560][0])
#                end= time.time()
#                print('time sec', end-start)
                if config.PREDICT_NORMAL_NP:
                    ...
                    gt_depth = gt_depth[:, 0]
                    depth_np_loss = l1LossMask(depth_np_pred[:, 80:560], gt_depth[:, 80:560], (gt_depth[:, 80:560] > 1e-4).float()) + depth_smooth_loss 
#                    depth_np_loss = l1LossMask(depth_np_pred[:, 80:560], gt_depth[:, 80:560], (gt_depth[:, 80:560] > 1e-4).float()) 
#                    print('depth_np_in', depth_np_loss)   
                    ...
                    depth_np_loss = l1LossMask(depth_np_pred[:, 80:560], gt_depth[:, 80:560], (gt_depth[:, 80:560] > 1e-4).float()) + depth_smooth_loss
#                    depth_np_loss = l1LossMask(depth_np_pred[:, 80:560], gt_depth[:, 80:560], (gt_depth[:, 80:560] > 1e-4).float()) 
#                    print('depth_np_else', depth_np_loss) 
                    losses.append(depth_np_loss)
                    normal_np_pred = None
                    pass
...
#                    print('here2')
                ...
                if 'depth' in options.suffix:
                    ...
                    if len(detections) > 0:
                        ...
                        print('sup_depth')
                        print('depth_np_pred',depth_np_pred)
                        print('shape',depth_np_pred.shape)
                        print('planexyz',plane_XYZ)
                        print('shape',plane_XYZ.shape)
                        print('cat_xyz',plane_XYZ[:, 1])
                        print('shape',plane_XYZ[:, 1].shape)
            ...
            for l in losses:
                status += '%0.5f '%l
                continue
            with open('test/logs/smooth.txt','a') as fl:
                fl.write('\r\n' + str(sampleIndex) + ' ' + status)
                fl.flush
